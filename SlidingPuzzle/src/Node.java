...
public class Node implements Comparable<Node> {
    ...
    public void calHCost() { this.hCost = calManDist(Main.targetState); }
    ...
    public int calManDist(char[][] goalState) {
        ...
        for(int i = 0; i<this.curState.length; i++) {
            for(int j = 0; j<this.curState.length; j++) {
                if (curState[i][j] == goalState[i][j] || curState[i][j] == '0') {
                    break;
                } else {
    ...
    public boolean isGoal() {
        return Arrays.deepEquals(Utils.arrayDeepClone(this.curState), Main.targetState);
    }
}